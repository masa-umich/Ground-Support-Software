...
class ThreeWayValve(AvionicsObject):
    ...
    def __init__(
        ...
        width: float = 40 * 1,
        height: float = 27 * 1,
        name: str = "3 Way Valve",
        scale: float = 1,
        serial_number: str = "",
        long_name: str = "3 Way Valve",
        ...
        self.current = 0
        self.sec_width = (
            18 * self.widget_parent.gui.pixel_scale_ratio[0]
        )  # width of a valve "section" same as 2 way valve width
    ...
    def draw(self):
        ...
        if self.state == 1:
            fill = [
                QBrush(Constants.fluidColor[self.fluid], Qt.SolidPattern),
                QBrush(Constants.fluidColor[self.fluid], Qt.SolidPattern),
                QBrush(0),
            ]
        else:
            fill = [
                QBrush(0),
                QBrush(Constants.fluidColor[self.fluid], Qt.Dense5Pattern),
                QBrush(Constants.fluidColor[self.fluid], Qt.Dense5Pattern),
            ]
    ...
    def setAnchorPoints(self):
        ...
        if self.is_vertical == False:
            self.anchor_points[0].updateLocalPosition(
                QPointF(0, int(self.sec_width / 2))
            )
            self.anchor_points[1].updateLocalPosition(
                QPointF(self.width, int(self.sec_width / 2))
            )
            ...
            self.anchor_points[0].updateLocalPosition(
                QPointF(int(self.sec_width / 2), 0)
            )
            self.anchor_points[1].updateLocalPosition(
                QPointF(int(self.sec_width / 2), self.height)
            )
    ...
    def toggle(self):
        ...
        if self.state == 0:
            ...
            print(
                "WARNING STATE OF ThreeWayValve "
                + str(self._id)
