...
### telemParse.py
### Autogenerated by firmware-libraries/SerialComms/python/telem_file_generator.py on Wed Mar  3 23:03:06 2021
...
class PressurizationController:
    def __init__(self):
        ...
        self.items[76] = "epot[0]"
        self.items[77] = "epot[1]"
        self.items[78] = "epot[2]"
        self.items[79] = "epot[3]"
        self.items[80] = "test_duration"
        self.items[81] = "micros"
        self.items[82] = "telem_rate"
        self.items[83] = "adc_rate"
        self.items[84] = "flash_mem"
        self.items[85] = "own_board_addr"
        ...
        self.units[self.items[80]] = "ms"
        self.units[self.items[81]] = "us"
        self.units[self.items[82]] = "Hz"
        self.units[self.items[83]] = "Hz"
        self.units[self.items[84]] = "bytes"
    ...
    def parse_packet(self, packet):
        ...
        self.dict[self.items[15]] = float(
            (float(struct.unpack("<h", packet[28:30])[0])) / 100
        )
        self.dict[self.items[16]] = float(
            (float(struct.unpack("<h", packet[30:32])[0])) / 100
        )
        self.dict[self.items[17]] = int(
            (float(struct.unpack("<B", packet[32:33])[0])) / 1
        )
        ...
        self.dict[self.items[36]] = float(
            (float(struct.unpack("<i", packet[51:55])[0])) / 100
        )
        self.dict[self.items[37]] = float(
            (float(struct.unpack("<i", packet[55:59])[0])) / 100
        )
        self.dict[self.items[38]] = float(
            (float(struct.unpack("<B", packet[59:60])[0])) / 100
        )
        self.dict[self.items[39]] = float(
            (float(struct.unpack("<B", packet[60:61])[0])) / 100
        )
        self.dict[self.items[40]] = float(
            (float(struct.unpack("<H", packet[61:63])[0])) / 1
        )
        self.dict[self.items[41]] = float(
            (float(struct.unpack("<H", packet[63:65])[0])) / 100
        )
        self.dict[self.items[42]] = float(
            (float(struct.unpack("<H", packet[65:67])[0])) / 100
        )
        self.dict[self.items[43]] = float(
            (float(struct.unpack("<H", packet[67:69])[0])) / 100
        )
        self.dict[self.items[44]] = float(
            (float(struct.unpack("<H", packet[69:71])[0])) / 100
        )
        self.dict[self.items[45]] = float(
            (float(struct.unpack("<H", packet[71:73])[0])) / 100
        )
        self.dict[self.items[46]] = float(
            (float(struct.unpack("<H", packet[73:75])[0])) / 100
        )
        ...
        self.dict[self.items[63]] = float(
            (float(struct.unpack("<H", packet[107:109])[0])) / 10
        )
        self.dict[self.items[64]] = float(
            (float(struct.unpack("<h", packet[109:111])[0])) / 10
        )
        self.dict[self.items[65]] = float(
            (float(struct.unpack("<h", packet[111:113])[0])) / 10
        )
        self.dict[self.items[66]] = float(
            (float(struct.unpack("<h", packet[113:115])[0])) / 10
        )
        self.dict[self.items[67]] = float(
            (float(struct.unpack("<h", packet[115:117])[0])) / 10
        )
        self.dict[self.items[68]] = float(
            (float(struct.unpack("<h", packet[117:119])[0])) / 10
        )
        self.dict[self.items[69]] = float(
            (float(struct.unpack("<h", packet[119:121])[0])) / 10
        )
        self.dict[self.items[70]] = float(
            (float(struct.unpack("<h", packet[121:123])[0])) / 10
        )
        self.dict[self.items[71]] = float(
            (float(struct.unpack("<h", packet[123:125])[0])) / 10
        )
        ...
        self.dict[self.items[76]] = float(
            (float(struct.unpack("<i", packet[133:137])[0])) / 10
        )
        self.dict[self.items[77]] = float(
            (float(struct.unpack("<i", packet[137:141])[0])) / 10
        )
        self.dict[self.items[78]] = float(
            (float(struct.unpack("<i", packet[141:145])[0])) / 10
        )
        self.dict[self.items[79]] = float(
            (float(struct.unpack("<i", packet[145:149])[0])) / 10
        )
        self.dict[self.items[80]] = int(
            (float(struct.unpack("<I", packet[149:153])[0])) / 1
        )
        self.dict[self.items[81]] = int(
            (float(struct.unpack("<Q", packet[153:161])[0])) / 1
        )
        self.dict[self.items[82]] = int(
            (float(struct.unpack("<I", packet[161:165])[0])) / 1
        )
        self.dict[self.items[83]] = int(
            (float(struct.unpack("<I", packet[165:169])[0])) / 1
        )
        self.dict[self.items[84]] = int(
            (float(struct.unpack("<I", packet[169:173])[0])) / 1
        )
