...
class GSEControllerCalibrations:
    ...
    def parse_packet(self, packet):
        ...
        self.dict[self.items[28]] = float(
            (float(struct.unpack("<i", packet[92:96])[0])) / 10000
        )
        self.dict[self.items[29]] = float(
            (float(struct.unpack("<i", packet[96:100])[0])) / 10000
        )
        ...
        self.dict[self.items[46]] = float(
            (float(struct.unpack("<h", packet[132:134])[0])) / 1
        )
        self.dict[self.items[47]] = float(
            (float(struct.unpack("<h", packet[134:136])[0])) / 1
        )
        self.dict[self.items[48]] = float(
            (float(struct.unpack("<h", packet[136:138])[0])) / 1
        )
        self.dict[self.items[49]] = float(
            (float(struct.unpack("<h", packet[138:140])[0])) / 1
        )
        self.dict[self.items[50]] = float(
            (float(struct.unpack("<h", packet[140:142])[0])) / 1
        )
        self.dict[self.items[51]] = float(
            (float(struct.unpack("<h", packet[142:144])[0])) / 1
        )
