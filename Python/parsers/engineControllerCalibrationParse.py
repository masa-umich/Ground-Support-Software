...
class EngineControllerCalibrations:
    ...
    def parse_packet(self, packet):
        ...
        self.dict[self.items[14]] = float(
            (float(struct.unpack("<i", packet[36:40])[0])) / 10000
        )
        self.dict[self.items[15]] = float(
            (float(struct.unpack("<i", packet[40:44])[0])) / 10000
        )
        self.dict[self.items[16]] = float(
            (float(struct.unpack("<i", packet[44:48])[0])) / 10000
        )
        self.dict[self.items[17]] = float(
            (float(struct.unpack("<i", packet[48:52])[0])) / 10000
        )
        self.dict[self.items[18]] = float(
            (float(struct.unpack("<i", packet[52:56])[0])) / 10000
        )
        self.dict[self.items[19]] = float(
            (float(struct.unpack("<i", packet[56:60])[0])) / 10000
        )
        self.dict[self.items[20]] = float(
            (float(struct.unpack("<i", packet[60:64])[0])) / 10000
        )
        self.dict[self.items[21]] = float(
            (float(struct.unpack("<i", packet[64:68])[0])) / 10000
        )
        self.dict[self.items[22]] = float(
            (float(struct.unpack("<i", packet[68:72])[0])) / 10000
        )
        self.dict[self.items[23]] = float(
            (float(struct.unpack("<i", packet[72:76])[0])) / 10000
        )
        self.dict[self.items[24]] = float(
            (float(struct.unpack("<i", packet[76:80])[0])) / 10000
        )
        self.dict[self.items[25]] = float(
            (float(struct.unpack("<i", packet[80:84])[0])) / 10000
        )
        self.dict[self.items[26]] = float(
            (float(struct.unpack("<i", packet[84:88])[0])) / 10000
        )
        self.dict[self.items[27]] = float(
            (float(struct.unpack("<i", packet[88:92])[0])) / 10000
        )
        self.dict[self.items[28]] = float(
            (float(struct.unpack("<h", packet[92:94])[0])) / 1
        )
        self.dict[self.items[29]] = float(
            (float(struct.unpack("<h", packet[94:96])[0])) / 1
        )
        self.dict[self.items[30]] = float(
            (float(struct.unpack("<h", packet[96:98])[0])) / 1
        )
        self.dict[self.items[31]] = float(
            (float(struct.unpack("<h", packet[98:100])[0])) / 1
        )
        self.dict[self.items[32]] = float(
            (float(struct.unpack("<h", packet[100:102])[0])) / 1
        )
        self.dict[self.items[33]] = float(
            (float(struct.unpack("<h", packet[102:104])[0])) / 1
        )
        self.dict[self.items[34]] = float(
            (float(struct.unpack("<h", packet[104:106])[0])) / 1
        )
        self.dict[self.items[35]] = float(
            (float(struct.unpack("<h", packet[106:108])[0])) / 1
        )
        self.dict[self.items[36]] = float(
            (float(struct.unpack("<h", packet[108:110])[0])) / 1
        )
        self.dict[self.items[37]] = float(
            (float(struct.unpack("<h", packet[110:112])[0])) / 1
        )
        self.dict[self.items[38]] = float(
            (float(struct.unpack("<h", packet[112:114])[0])) / 1
        )
        self.dict[self.items[39]] = float(
            (float(struct.unpack("<h", packet[114:116])[0])) / 1
        )
        self.dict[self.items[40]] = float(
            (float(struct.unpack("<h", packet[116:118])[0])) / 1
        )
        self.dict[self.items[41]] = float(
            (float(struct.unpack("<h", packet[118:120])[0])) / 1
        )
        self.dict[self.items[42]] = float(
            (float(struct.unpack("<h", packet[120:122])[0])) / 1
        )
        self.dict[self.items[43]] = float(
            (float(struct.unpack("<h", packet[122:124])[0])) / 1
        )
        self.dict[self.items[44]] = float(
            (float(struct.unpack("<h", packet[124:126])[0])) / 1
        )
        self.dict[self.items[45]] = float(
            (float(struct.unpack("<h", packet[126:128])[0])) / 1
        )
        self.dict[self.items[46]] = float(
            (float(struct.unpack("<h", packet[128:130])[0])) / 1
        )
        self.dict[self.items[47]] = float(
            (float(struct.unpack("<h", packet[130:132])[0])) / 1
        )
