...
### telemParse.py
### Autogenerated by firmware-libraries/SerialComms/python/telem_file_generator.py on Sat Apr 24 01:18:39 2021
...
class PressurizationController:
    ...
    def parse_packet(self, packet):
        ...
        self.dict[self.items[9]] = float(
            (float(struct.unpack("<i", packet[16:20])[0])) / 10
        )
        self.dict[self.items[10]] = float(
            (float(struct.unpack("<i", packet[20:24])[0])) / 10
        )
        self.dict[self.items[11]] = float(
            (float(struct.unpack("<i", packet[24:28])[0])) / 10
        )
        self.dict[self.items[12]] = float(
            (float(struct.unpack("<i", packet[28:32])[0])) / 10
        )
        ...
        self.dict[self.items[15]] = float(
            (float(struct.unpack("<h", packet[40:42])[0])) / 100
        )
        self.dict[self.items[16]] = float(
            (float(struct.unpack("<h", packet[42:44])[0])) / 100
        )
        self.dict[self.items[17]] = int(
            (float(struct.unpack("<B", packet[44:45])[0])) / 1
        )
        ...
        self.dict[self.items[23]] = float(
            (float(struct.unpack("<B", packet[50:51])[0])) / 10
        )
        self.dict[self.items[24]] = float(
            (float(struct.unpack("<B", packet[51:52])[0])) / 10
        )
        self.dict[self.items[25]] = float(
            (float(struct.unpack("<B", packet[52:53])[0])) / 10
        )
        self.dict[self.items[26]] = float(
            (float(struct.unpack("<B", packet[53:54])[0])) / 10
        )
        self.dict[self.items[27]] = float(
            (float(struct.unpack("<B", packet[54:55])[0])) / 10
        )
        self.dict[self.items[28]] = float(
            (float(struct.unpack("<B", packet[55:56])[0])) / 10
        )
        ...
        self.dict[self.items[36]] = float(
            (float(struct.unpack("<i", packet[63:67])[0])) / 100
        )
        self.dict[self.items[37]] = float(
            (float(struct.unpack("<i", packet[67:71])[0])) / 100
        )
        self.dict[self.items[38]] = float(
            (float(struct.unpack("<B", packet[71:72])[0])) / 100
        )
        self.dict[self.items[39]] = float(
            (float(struct.unpack("<B", packet[72:73])[0])) / 100
        )
        self.dict[self.items[40]] = float(
            (float(struct.unpack("<H", packet[73:75])[0])) / 1
        )
        self.dict[self.items[41]] = float(
            (float(struct.unpack("<H", packet[75:77])[0])) / 100
        )
        self.dict[self.items[42]] = float(
            (float(struct.unpack("<H", packet[77:79])[0])) / 100
        )
        self.dict[self.items[43]] = float(
            (float(struct.unpack("<H", packet[79:81])[0])) / 100
        )
        self.dict[self.items[44]] = float(
            (float(struct.unpack("<H", packet[81:83])[0])) / 100
        )
        self.dict[self.items[45]] = float(
            (float(struct.unpack("<H", packet[83:85])[0])) / 100
        )
        self.dict[self.items[46]] = float(
            (float(struct.unpack("<H", packet[85:87])[0])) / 100
        )
        self.dict[self.items[47]] = float(
            (float(struct.unpack("<H", packet[87:89])[0])) / 100
        )
        self.dict[self.items[48]] = float(
            (float(struct.unpack("<H", packet[89:91])[0])) / 100
        )
        self.dict[self.items[49]] = float(
            (float(struct.unpack("<H", packet[91:93])[0])) / 100
        )
        self.dict[self.items[50]] = float(
            (float(struct.unpack("<H", packet[93:95])[0])) / 100
        )
        self.dict[self.items[51]] = float(
            (float(struct.unpack("<H", packet[95:97])[0])) / 100
        )
        self.dict[self.items[52]] = float(
            (float(struct.unpack("<h", packet[97:99])[0])) / 10
        )
        self.dict[self.items[53]] = float(
            (float(struct.unpack("<h", packet[99:101])[0])) / 10
        )
        self.dict[self.items[54]] = int(
            (float(struct.unpack("<h", packet[101:103])[0])) / 1
        )
        self.dict[self.items[55]] = int(
            (float(struct.unpack("<h", packet[103:105])[0])) / 1
        )
        self.dict[self.items[56]] = float(
            (float(struct.unpack("<i", packet[105:109])[0])) / 100
        )
        self.dict[self.items[57]] = float(
            (float(struct.unpack("<i", packet[109:113])[0])) / 100
        )
        self.dict[self.items[58]] = float(
            (float(struct.unpack("<H", packet[113:115])[0])) / 100
        )
        self.dict[self.items[59]] = float(
            (float(struct.unpack("<H", packet[115:117])[0])) / 100
        )
        self.dict[self.items[60]] = float(
            (float(struct.unpack("<H", packet[117:119])[0])) / 100
        )
        self.dict[self.items[61]] = float(
            (float(struct.unpack("<H", packet[119:121])[0])) / 100
        )
        self.dict[self.items[62]] = float(
            (float(struct.unpack("<H", packet[121:123])[0])) / 100
        )
        self.dict[self.items[63]] = float(
            (float(struct.unpack("<H", packet[123:125])[0])) / 100
        )
        self.dict[self.items[64]] = float(
            (float(struct.unpack("<h", packet[125:127])[0])) / 10
        )
        self.dict[self.items[65]] = float(
            (float(struct.unpack("<h", packet[127:129])[0])) / 10
        )
        self.dict[self.items[66]] = float(
            (float(struct.unpack("<h", packet[129:131])[0])) / 10
        )
        self.dict[self.items[67]] = float(
            (float(struct.unpack("<h", packet[131:133])[0])) / 10
        )
        self.dict[self.items[68]] = float(
            (float(struct.unpack("<h", packet[133:135])[0])) / 10
        )
        self.dict[self.items[69]] = float(
            (float(struct.unpack("<h", packet[135:137])[0])) / 10
        )
        self.dict[self.items[70]] = float(
            (float(struct.unpack("<i", packet[137:141])[0])) / 100
        )
        self.dict[self.items[71]] = float(
            (float(struct.unpack("<i", packet[141:145])[0])) / 100
        )
        self.dict[self.items[72]] = float(
            (float(struct.unpack("<i", packet[145:149])[0])) / 100
        )
        self.dict[self.items[73]] = float(
            (float(struct.unpack("<i", packet[149:153])[0])) / 100
        )
        self.dict[self.items[74]] = float(
            (float(struct.unpack("<i", packet[153:157])[0])) / 100
        )
        self.dict[self.items[75]] = float(
            (float(struct.unpack("<i", packet[157:161])[0])) / 100
        )
        self.dict[self.items[76]] = float(
            (float(struct.unpack("<i", packet[161:165])[0])) / 10
        )
        self.dict[self.items[77]] = float(
            (float(struct.unpack("<i", packet[165:169])[0])) / 10
        )
        self.dict[self.items[78]] = float(
            (float(struct.unpack("<i", packet[169:173])[0])) / 10
        )
        self.dict[self.items[79]] = float(
            (float(struct.unpack("<i", packet[173:177])[0])) / 10
        )
        self.dict[self.items[80]] = int(
            (float(struct.unpack("<B", packet[177:178])[0])) / 1
        )
        self.dict[self.items[81]] = int(
            (float(struct.unpack("<B", packet[178:179])[0])) / 1
        )
        self.dict[self.items[82]] = int(
            (float(struct.unpack("<I", packet[179:183])[0])) / 1
        )
        self.dict[self.items[83]] = int(
            (float(struct.unpack("<I", packet[183:187])[0])) / 1
        )
        self.dict[self.items[84]] = int(
            (float(struct.unpack("<i", packet[187:191])[0])) / 1
        )
        self.dict[self.items[85]] = int(
            (float(struct.unpack("<Q", packet[191:199])[0])) / 1
        )
        self.dict[self.items[86]] = int(
            (float(struct.unpack("<I", packet[199:203])[0])) / 1
        )
        self.dict[self.items[87]] = int(
            (float(struct.unpack("<I", packet[203:207])[0])) / 1
        )
        self.dict[self.items[88]] = int(
            (float(struct.unpack("<I", packet[207:211])[0])) / 1
        )
        self.dict[self.items[89]] = int(
            (float(struct.unpack("<B", packet[211:212])[0])) / 1
        )
        self.dict[self.items[90]] = int(
