...
### Autogenerated by firmware-libraries/SerialComms/python/cmd_file_generator.py on Mon Apr 11 23:34:38 2022
...
class _S2_InterfaceAutogen:
    def __init__(self):
        ...
        self.cmd_args_dict = {
            ...
            49: [
                ("pt_num", "uint8_t", 1),
                ("mV_per_psi", "float", 10000),
                ("mV_offset", "float", 1),
            ],
            50: [("abort_enable_status", "uint8_t", 1)],
            51: [],
            52: [("enable_tank_vent", "uint8_t", 1)],
        }
    ...
    def s2_command(self, ser, cmd_info):
        ...
        if cmd_info["function_name"] not in self.cmd_names_dict.keys():
            print(
                ...
                + " is not a valid function name. No command was sent."
            )
            return
        ...
        packet = [
            0
        ] * 12  # Header is 12 bytes, will add bytes to fit function arguments
        ...
        # set_pt_slope_offset()
            ...
            packet.append((int(cmd_info["args"][0] * 1) >> 0) & 0xFF)
            # mV_per_psi
            packet.append((int(cmd_info["args"][1] * 10000) >> 0) & 0xFF)
            packet.append((int(cmd_info["args"][1] * 10000) >> 8) & 0xFF)
            packet.append((int(cmd_info["args"][1] * 10000) >> 16) & 0xFF)
            packet.append((int(cmd_info["args"][1] * 10000) >> 24) & 0xFF)
            # mV_offset
            packet.append((int(cmd_info["args"][2] * 1) >> 0) & 0xFF)
            packet.append((int(cmd_info["args"][2] * 1) >> 8) & 0xFF)
            packet.append((int(cmd_info["args"][2] * 1) >> 16) & 0xFF)
            packet.append((int(cmd_info["args"][2] * 1) >> 24) & 0xFF)
