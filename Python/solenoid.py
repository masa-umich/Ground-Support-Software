...
class Solenoid(AvionicsObject):
    ...
    def _initAnchorPoints(self):
        ...
        if self.is_vertical:
            ...
            anchor_points = [
                AnchorPoint(
                    QPoint(0, int(sol_mid_point)), self, 0, parent=self.widget_parent
                ),
                AnchorPoint(
    ...
    def draw(self):
        ...
        if self.state == 1 and self.normally_open is False:
            self.widget_parent.painter.setBrush(
                Constants.fluidColor[self.fluid]
            )  # This function colors in a path
        ...
        if self.is_vertical is False:
            ...
            path.addRect(
                self.width / 4, 0, self.width / 2, coil_height
            )  # left cord, width height
            ...
            path.addRect(
                sol_width / 2 + coil_width, self.height / 4, coil_width, self.height / 2
            )  # left cord, width height
    ...
    def generateSaveDict(self):
        ...
        save_dict = {"normally open": self.normally_open, "keybind": self.keybind}
    ...
    def updateFromDataPacket(self, data_packet: dict):
        ...
        if self.isAvionicsFullyDefined():
            ...
            current = (
                data_packet[channel_name + ".i"]
                if (channel_name + ".i") in data_packet
                else 0
            )
