...
class MissionWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.CET_static_tooltip = (
            ""  # Random var to track ending portion of tooltip. More below
        )
        ...
        self.CET_label.move(
            0, int(0 - 4 * self.gui.pixel_scale_ratio[1])
        ...
        self.titleLabel.move(
            int(self.dateTimeLabelRPos + self.underBarWBuffer), int(self.mainHeight - 1)
        )
        self.titleLabel.show()

        # Set all the indicators, move into position
        self.campaignIndicator = IndicatorLightWidget(
            self, self.gui, "Campaign", 20, "Red", 14, 30, 5, 2
        )
        ...
        self.connectionIndicator = IndicatorLightWidget(
            self, self.gui, "Connection", 20, "Red", 14, 30, 5, 2
        )
        self.connectionIndicator.move(
            int(self.campaignIndicator.pos().x() + self.campaignIndicator.width()), 0
        )
        self.connectionIndicator.setToolTip("No connection")

        self.commandIndicator = IndicatorLightWidget(
            self, self.gui, "Command", 20, "Red", 14, 30, 5, 2
        )
        self.commandIndicator.move(
            self.connectionIndicator.pos().x() + self.connectionIndicator.width(), 0
        )
        self.commandIndicator.setToolTip("In command")

        self.systemIndicator = IndicatorLightWidget(
            self, self.gui, "System", 20, "Green", 14, 30, 5, 1
        )
        self.systemIndicator.move(
            self.commandIndicator.pos().x() + self.commandIndicator.width(), 0
        )
        ...
        self.status_label.setFixedWidth(
            int(
                self.width
                - (self.systemIndicator.pos().x() + self.systemIndicator.width())
            )
        )
        self.status_label.setAlignment(Qt.AlignLeft | Qt.AlignVCenter)
        self.status_label.move(
            int(
                self.systemIndicator.pos().x()
                + self.systemIndicator.width()
                + self.mainBarWBuffer
            ),
            int(0 - 4 * self.gui.pixel_scale_ratio[1]),
        )  # Nasty but makes it look more centered
    ...
    def generateCETAsText(CET_time, test_start_time: int = None):
        ...
        qtime = QTime(
            0, 0, 0
        )  # Can't pass in directly because the initializer does not like secs above 59
    ...
    def updateCETLabel(self, CET_time, test_start_time: int):
        ...
        if self.gui.campaign.isTestActive:
            self.CET_label.setToolTip(
                "Current "
                + self.generateCETAsText(self.gui.campaign.CET)
                + "\n"
                + self.CET_static_tooltip
            )
    ...
    def updateStatusLabel(self, status: str, is_warning: bool = False):
        ...
        if is_warning:
            self.status_label.setStyleSheet(
                "color:" + Constants.MASA_Maize_color.name()
            )
        else:
            self.status_label.setStyleSheet("color: white")
    ...
    def updateDateTimeLabel(self):
        ...
        dateTimeString = (
            currentDateTime.toString("MMMM dd")
            + dayString
            + currentDateTime.toString("yyyy, hh:mmap")
        )
    ...
    def updateWidgetOnTestEnd(self):
        ...
        self.CET_static_tooltip = (
            self.CET_label.toolTip()
            + f'{"   |   end: "+ QDateTime.currentDateTime().time().toString("h:mmap") + " ("+self.generateCETAsText(self.gui.campaign.CET) + ")":<26}'
        )
        self.CET_label.setToolTip(
            "Current "
            + self.generateCETAsText(self.gui.campaign.CET)
    ...
    def updateWidgetOnCampaignStart(self):
        ...
        self.CET_label.setToolTip(
            "Campaign start: "
            + self.gui.campaign.startDateTime.time().toString("h:mmap")
            + " (CET-00:00:00)\n"
        )
        self.CET_static_tooltip = (
            "Campaign start: "
    ...
    def paintEvent(self, e):
        ...
        if not self.titleLabel.text() == "":
            path.moveTo(
                self.dateTimeLabel.width() + self.underBarWBuffer, self.height - 1
            )
            path.lineTo(
                self.dateTimeLabelRPos
                + self.underBarWBuffer
    ...
    def updateConnectionStatus(
        ...
        # Server to GUI connection bad, no info to display at the time
            ...
            self.connectionIndicator.setIndicatorColor("Yellow")
            self.connectionIndicator.setToolTip(
                "Odd. Packets have no data but data handler insists connection is active"
            )
...
class MissionWidgetBackgroundThread(QThread):
    ...
    def run(self):
        ...
        while True:
            ...
            if (
                len(
                    self.missionWidget.centralWidget.controlsSidebarWidget.board_objects
                )
                == 0
            ):
                general_status_tooltip[
                    1
                ] = "No avionics boards added (Edit->Add Avionics)\n"
            ...
            for object_ in self.missionWidget.controlsWidget.object_list:
                ...
                if status > 0:
                    ...
                    object_status_tooltip[status] = (
                        object_status_tooltip[status] + message + "\n"
                    )
            ...
            if (
                len(general_status_tooltip[1]) > 0 or len(object_status_tooltip[1])
            ) > 0 and (
                len(general_status_tooltip[2]) > 0 or len(object_status_tooltip[2]) > 0
            ):
                tooltip = (
                    "Critical: \n"
                    + general_status_tooltip[2]
                    + object_status_tooltip[2]
                    + "\nWarnings: \n"
                    + general_status_tooltip[1]
                    + object_status_tooltip[1]
                )
            elif (
                len(general_status_tooltip[1]) > 0 or len(object_status_tooltip[1]) > 0
            ):
                tooltip = (
                    "Warnings: \n"
                    + general_status_tooltip[1]
