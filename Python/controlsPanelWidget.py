...
class ControlsPanelWidget(QWidget):
    ...
    def initEditFrame(self):
        ...
        self.edit_frame.resize(
            int(300 * self.gui.pixel_scale_ratio[0]), int(self.gui.screenResolution[1])
        )
        ...
        self.long_name_visibility_group = QButtonGroup(
            self
        )  # Not really needed here but including it
        ...
        label = QLabel(
            "Component Parameters:                                                                  "
        )  # TODO: jankity jank jank
        label.setFont(title_font)
        label.setStyleSheet("color: white")
        self.edit_form_layout.addRow(label)
        self.createLineEdit(
            self.component_name_textbox, "Component Name", "Component Name:"
        )
        self.createLineEdit(
            self.serial_number_textbox, "Serial Number", "Serial Number:"
        )
        self.is_pos_locked_group = self.createTFRadioButtons(
            "Component Fixed?", "Position:", "Locked", "Unlocked", False
        )
        self.createSpinbox(
            self.scale_spinbox, "Component Scale", "Scale:", 0.1, 10, 0.1
        )
        self.createComboBox(
            self.fluid_combobox, "Component Fluid", "Fluid:", Constants.fluids
        )
        self.createComboBox(
            self.board_combobox, "Board", "Board:", ["Undefined"] + Constants.boards
        )
        self.createComboBox(
        ...
        label.setStyleSheet("color: white")
        self.edit_form_layout.addRow(label)
        self.long_name_visibility_group = self.createTFRadioButtons(
            "Component Name Visibility", "Visibility:", "Shown", "Hidden", True
        )
        self.createComboBox(
            self.long_name_position_combobox,
            "Component Name Position",
            "Position:",
            ["Top", "Right", "Bottom", "Left", "Custom"],
        )
        self.createSpinbox(
            self.long_name_font_size_spinbox,
        ...
        self.createSpinbox(
            self.long_name_row_spinbox, "Component Name Rows", "Text Rows:", 1, 5, 1
        )
        self.edit_form_layout.addRow(QLabel(""))
        ...
        label.setStyleSheet("color: white")
        self.edit_form_layout.addRow(label)
        self.serial_number_visibility_group = self.createTFRadioButtons(
            "Serial Number Visibility", "Visibility:", "Shown", "Hidden", True
        )
        self.createComboBox(
            self.serial_number_position_combobox,
            "Serial Number Position",
            "Position:",
            ["Top", "Right", "Bottom", "Left", "Custom"],
        )
        self.createSpinbox(
            self.serial_number_font_size_spinbox,
        ...
        self.component_prop_label = QLabel(
        ...
        self.tank_override_label = QLabel(
            "Component Properties:                                                                  "
        )
        ...
        self.createCheckbox(
            self.tank_override_checkbox,
            "Tank Override",
            "Override Systems Indicator Light",
        )
    ...
    def createLineEdit(
        ...
        lineEdit.textChanged.connect(
            lambda: self.updateEditingObjectFields(lineEdit.text(), identifier)
        )
    ...
    def createTFRadioButtons(
        self,
        identifier,
        label_text: str,
        true_btn_label="True",
        false_btn_label="False",
        checked: bool = True,
    ):
        """
        ...
        true_button.toggled.connect(
            lambda: self.updateEditingObjectFields(True, identifier)
        )
        false_button.toggled.connect(
            lambda: self.updateEditingObjectFields(False, identifier)
        )
    ...
    def createComboBox(
        self, comboBox: QComboBox, identifier: str, label_text: str, items: []
    ):
        """
        ...
        comboBox.currentIndexChanged.connect(
            lambda: self.updateEditingObjectFields(comboBox.currentText(), identifier)
        )
    ...
    def createSpinbox(
        ...
        spinBox.valueChanged.connect(
            lambda: self.updateEditingObjectFields(spinBox.value(), identifier)
        )
    ...
    def createCheckbox(self, checkBox: QCheckBox, identifier: str, label_text: str):
        ...
        checkBox.stateChanged.connect(
            lambda: self.updateEditingObjectFields(
                str(checkBox.isChecked()), identifier
            )
        )
    ...
    def updateEditPanelFields(self, object_):
        ...
        try:
            ...
            if board_name in Constants.boards:
                ...
                self.valve_channels = [
                    str(x) for x in range(0, self.interface.num_valves[addr])
                ]
                self.motor_channels = [
                    str(x) for x in range(0, self.interface.num_motors[addr])
                ]
                self.tank_channels = [
                    str(x) for x in range(0, self.interface.num_tanks[addr])
                ]
            else:
                self.valve_channels = []
                self.motor_channels = []
            ...
            if (
                object_.object_name == "Solenoid"
                or object_.object_name == "3 Way Valve"
            ):
                self.comboBoxReplaceFields(
                    self.channel_combobox, ["Undefined"] + self.valve_channels
                )
                self.channel_combobox.setEditable(False)
                self.setSolenoidComponentPropertiesVisibility(True)
            elif object_.object_name == "Motor":
                self.comboBoxReplaceFields(
                    self.channel_combobox, ["Undefined"] + self.motor_channels
                )
                self.channel_combobox.setEditable(False)
            elif object_.object_name == "Tank":
                self.comboBoxReplaceFields(
                    self.channel_combobox, ["Undefined"] + self.tank_channels
                )
                self.channel_combobox.setEditable(False)
                ...
                if board_name in Constants.boards:
                    ...
                    specific_channels = [
                        i for i in self.sensor_channels if i.startswith(prefix)
                    ]
                else:
                    specific_channels = []

                self.comboBoxReplaceFields(
                    self.channel_combobox, ["Undefined"] + specific_channels
                )
                self.channel_combobox.setEditable(True)
        ...
        self.long_name_position_combobox.setCurrentText(
            object_.long_name_label.position_string
        )
        self.setTFRadioButtonValue(
            self.long_name_visibility_group, object_.long_name_label.isVisible()
        )
        self.setTFRadioButtonValue(
            self.serial_number_visibility_group, object_.serial_number_label.isVisible()
        )
        ...
        self.serial_number_position_combobox.setCurrentText(
            object_.serial_number_label.position_string
        )
        ...
        self.serial_number_font_size_spinbox.setValue(
            object_.serial_number_label.getFontSize()
        )

        # Enables the board and channel box and sets their values
        if (
            object_.object_name == "Generic Sensor"
            or object_.object_name == "Solenoid"
            or object_.object_name == "3 Way Valve"
            or object_.object_name == "Motor"
            or object_.object_name == "Tank"
        ):
            ...
            # Component Label Parameters
                ...
                for object_ in self.editing_object_list:
                    ...
                    self.gui.setStatusBarMessage(
                        ...
                        + object_.long_name
                        + ")"
                        + ": set component name position to "
                        + str(text)
                    )
                ...
                for object_ in self.editing_object_list:
                    ...
                    self.gui.setStatusBarMessage(
                        ...
                        + object_.long_name
                        + ")"
                        + ": set component name font size to "
                        + str(text)
                    )
    ...
    def setSolenoidComponentPropertiesVisibility(self, is_vis):
        ...
        :param is_vis:
        """
        sol_NC_NO_index = self.edit_form_layout.getWidgetPosition(
            self.solenoid_NC_NO_combobox
        )
        if is_vis:
            ...
            self.edit_form_layout.itemAt(
                sol_NC_NO_index[0], sol_NC_NO_index[1] - 1
            ).widget().show()
        else:
    ...
    def setTankOverridePropertyVisibility(self, is_vis):
        ...
        :param is_vis:
        """
        tank_override_index = self.edit_form_layout.getWidgetPosition(
            self.tank_override_checkbox
        )
        if is_vis:
            ...
            self.edit_form_layout.itemAt(
                tank_override_index[0], tank_override_index[1] - 1
            ).widget().show()
        else:
