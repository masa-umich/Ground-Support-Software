...
class BaseGui(QObject):
    ...
    def __init__(self, qapp: QApplication, mainWindow: QWindow = None):
        ...
        # If this is the first time running, create required directories, and guess at scaling values
            ...
            self.workspace_path = str(
                QFileDialog.getExistingDirectory(
                    None, "Select Workspace Directory", options=options
                )
            )
            ...
            readMe.write(
                "This is the directory that the GUI pulls startup files from, do not delete, "
            ...
            self.pixel_scale_ratio = [
                self.screenResolution[0] / 1600,
                self.screenResolution[1] / 1200,
            ]
        ...
        QFontDatabase.addApplicationFont(
            self.LAUNCH_DIRECTORY + "Fonts/Montserrat/Montserrat-Medium.ttf"
        )
        QFontDatabase.addApplicationFont(
            self.LAUNCH_DIRECTORY + "Fonts/RobotoMono/RobotoMono-Regular.ttf"
        )
        QFontDatabase.addApplicationFont(
            self.LAUNCH_DIRECTORY + "Fonts/RobotoMono/RobotoMono-Light.ttf"
        )
        QFontDatabase.addApplicationFont(
            self.LAUNCH_DIRECTORY + "Fonts/RobotoMono/RobotoMono-Medium.ttf"
        )
        QFontDatabase.addApplicationFont(
            self.LAUNCH_DIRECTORY + "Fonts/RobotoMono/RobotoMono-Thin.ttf"
        )
        QFontDatabase.addApplicationFont(
            self.LAUNCH_DIRECTORY + "Fonts/RobotoMono/RobotoMono-Italic.ttf"
        )

        # This is a handler for the Client, which receives data and sends commands
        self.liveDataHandler = LiveDataHandler(self)
        self.liveDataHandler.getClient().gotConnectionToServerSignal.connect(
            self.gotConnectionToServer
        )
        self.liveDataHandler.getClient().serverDisconnectSignal.connect(
    ...
    def setMainWindow(self, mainWindow: QMainWindow):
        ...
        self._mainWindow.setWindowTitle(
            self._mainWindow.windowTitle() + " (" + Constants.GUI_VERSION + ")"
        )
    ...
    def reportIssue():
        ...
        """
        webbrowser.open(
            "https://gitlab.eecs.umich.edu/masa/avionics/gui/-/issues/new?issue%5Bmilestone_id%5D="
        )
    ...
    def openWiki():
        ...
        """
        webbrowser.open(
            "https://gitlab.eecs.umich.edu/masa/avionics/gui/-/wikis/GUI-Main-Page"
        )
    ...
    def savePreferences(self):
        ...
        preferences = {
            ...
            "pixel_scale_ratio": {
                "x": self.pixel_scale_ratio[0],
                "y": self.pixel_scale_ratio[1],
            },
    ...
    def loadPreferences(self):
        ...
        self.pixel_scale_ratio = [
            prefs["pixel_scale_ratio"]["x"],
            prefs["pixel_scale_ratio"]["y"],
        ]
        ...
        if not os.path.isdir(self.workspace_path):
            ...
            self.workspace_path = str(
                QFileDialog.getExistingDirectory(
                    None,
                    "Workspace not found: Select Workspace Directory",
                    options=options,
                )
            )
