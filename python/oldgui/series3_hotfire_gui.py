...
#flight_window.setGeometry(500,500,0,0)
    ...
    try:
        if(True):
            ...
            if not command_queue.empty():
                command = client_command(clientid, 3, args=(command_queue.get()))
            else:
            ...
            if(parser.BOARD_ID == TARGET_ADDRESS_FLIGHT):
                ...
                fs_label.setText("Logging Staus: " + str(parser.LOGGING_ACTIVE))
...
fs_label = QtGui.QLabel("Logging Staus: ")
flight_layout.addWidget(fs_label, zr+26, zc+9)

init_fs = QtGui.QPushButton("Init Flash")
flight_layout.addWidget(init_fs, zr+27, zc+9)
init_fs.clicked.connect(lambda: s2_command(TARGET_ADDRESS_FLIGHT, COMMAND_INIT_FS, 0, []))

start_fs = QtGui.QPushButton("Start Flash")
flight_layout.addWidget(start_fs, zr+28, zc+9)
start_fs.clicked.connect(lambda: s2_command(TARGET_ADDRESS_FLIGHT, COMMAND_LOG_START, 0, []))


end_fs = QtGui.QPushButton("End Flash")
flight_layout.addWidget(end_fs, zr+29, zc+9)
end_fs.clicked.connect(lambda: s2_command(TARGET_ADDRESS_FLIGHT, COMMAND_LOG_END, 0, []))

send_fs = QtGui.QPushButton("Send Data")
flight_layout.addWidget(send_fs, zr+30, zc+9)
send_fs.clicked.connect(lambda: get_file(TARGET_ADDRESS_FLIGHT))

#Disable these buttons for now, till flash is ready
#init_fs.setEnabled(False)
#start_fs.setEnabled(False)
#end_fs.setEnabled(False)
...
timer2.start(50) # 100hz for 10 as arg
